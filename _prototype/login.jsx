...
/* Microsoft "M" logo — official 4-square grid */
function MicrosoftLogo({ size = 20 }) {
  const s = size / 2 - 1;
  return (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none" style={{ flex: "none" }}>
      <rect x="1"  y="1"  width="8.5" height="8.5" fill="#F25022" rx="0.5" />
      <rect x="10.5" y="1"  width="8.5" height="8.5" fill="#7FBA00" rx="0.5" />
      <rect x="1"  y="10.5" width="8.5" height="8.5" fill="#00A4EF" rx="0.5" />
      <rect x="10.5" y="10.5" width="8.5" height="8.5" fill="#FFB900" rx="0.5" />
    </svg>
  );
}

function LoginScreen({ onEnter, theme, setTheme }) {
  const isMobile = !!(window.__forceMobile || window.innerWidth < 860);
  ...
  const signInWithMicrosoft = () => {
    setMsLoading(true);
    // Simulate OAuth redirect + return (~1.4s)
    setTimeout(() => { setMsLoading(false); onEnter(); }, 1400);
  };

  return (
    <div data-screen-label="Login" style={{ flex: 1,
      ...
      className="cp-login">
      {/* brand panel */}
      <div style={{ position: "relative", overflow: "hidden", padding: isMobile ? "32px 28px 24px" : "48px 52px",
        background: "linear-gradient(155deg, #0b1120 0%, #131a2e 60%, #1a1340 100%)", color: "#fff",
        display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        <Logo size={30} textColor="#fff" />
        <div style={{ position: "relative", zIndex: 2 }}>
          <h1 className="display" style={{ fontSize: isMobile ? 28 : 44, fontWeight: 700, lineHeight: 1.08, margin: "0 0 12px",
            letterSpacing: "-0.03em" }}>
            {isMobile ? <React.Fragment>Campus events,<br /><span style={{ background: "linear-gradient(90deg, var(--cat-tech), var(--cat-cultural), var(--cat-social))", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>one place.</span></React.Fragment>
            : <React.Fragment>Everything<br />happening on<br />campus, in one<br /><span style={{ background: "linear-gradient(90deg, var(--cat-tech), var(--cat-cultural), var(--cat-social))", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>living calendar.</span></React.Fragment>}
          </h1>
          {!isMobile && <p style={{ fontSize: 16, lineHeight: 1.6, color: "rgba(255,255,255,.7)", maxWidth: 380, margin: 0 }}>
            Discover fests, talks, matches and meetups. RSVP in a tap. Submit your own in seconds — we'll even read the poster for you.
          </p>}
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: isMobile ? 14 : 26 }}>
            {CATEGORIES.slice(0, isMobile ? 4 : 7).map((c) => (
          ...
          {!sent ? (
            <React.Fragment>
              ...
              <p style={{ margin: "0 0 24px", fontSize: 14.5, color: "var(--text-2)" }}>
                Use your IITG Microsoft account — one tap and you're in.
              </p>

              {/* ── Microsoft hero button ── */}
              <button onClick={signInWithMicrosoft} disabled={msLoading} style={{
                width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 12,
                padding: "14px 20px", borderRadius: "var(--r-md)", fontSize: 15.5, fontWeight: 700,
                fontFamily: "var(--font-sans)", cursor: msLoading ? "wait" : "pointer",
                border: "1.5px solid var(--border-strong)",
                background: msLoading ? "var(--surface-2)" : "var(--surface)",
                color: "var(--text)", boxShadow: "var(--shadow-sm)",
                transition: "all .15s ease",
              }}
                onMouseEnter={e => { if (!msLoading) e.currentTarget.style.background = "var(--surface-2)"; }}
                onMouseLeave={e => { e.currentTarget.style.background = msLoading ? "var(--surface-2)" : "var(--surface)"; }}>
                {msLoading ? (
                  <React.Fragment>
                    <span style={{ width: 20, height: 20, borderRadius: 99, border: "2.5px solid var(--border-strong)",
                      borderTopColor: "var(--cat-tech)", animation: "cp-spin .7s linear infinite", flex: "none" }} />
                    Signing in…
                  </React.Fragment>
                ) : (
                  <React.Fragment>
              ...
              {/* email notice */}
              <p style={{ margin: "12px 2px 0", fontSize: 12.5, color: "var(--text-3)", lineHeight: 1.5 }}>
                Uses your <span className="mono" style={{ fontSize: 12 }}>@iitg.ac.in</span> Outlook account.
                Single sign-on — no separate password needed.
              </p>
              ...
              {/* magic link toggle */}
              {!showMagic ? (
                <button onClick={() => setShowMagic(true)} style={{
                  width: "100%", padding: "12px 16px", borderRadius: "var(--r-md)", fontSize: 14.5,
                  fontWeight: 600, border: "1px solid var(--border)", background: "transparent",
                  color: "var(--text-2)", cursor: "pointer", display: "flex", alignItems: "center",
                  justifyContent: "center", gap: 8,
                }}>
                  <Icon name="mail" size={16} /> Use email magic link instead
                </button>
              ) : (
                <div>
                  ...
                  <div style={{ position: "relative", marginBottom: 12 }}>
                    ...
                    <input value={email} onChange={(e) => setEmail(e.target.value)}
                      placeholder="rollno@iitg.ac.in" type="email"
                      style={{ ...inp, paddingLeft: 40 }} autoFocus
                  ...
                  <Button variant="primary" full size="md" iconRight="arrowR"
                    disabled={!valid} onClick={() => setSent(true)}>Send magic link</Button>
                </div>
              )}

              <Divider />
              <Button variant="ghost" full onClick={() => onEnter()}
                style={{ color: "var(--text-3)", fontSize: 13.5 }}>Browse as guest</Button>
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <button onClick={() => { setSent(false); }} style={{ border: "none", background: "none",
                marginTop: 16, fontSize: 13, fontWeight: 600, color: "var(--text-3)", cursor: "pointer" }}>
