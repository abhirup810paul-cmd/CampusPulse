...
import { parseDate, longDate, relativeDay, fmtRange, generateICS, getGoogleCalendarUrl } from '@/lib/utils';
import { avatars } from '@/lib/data';
import { Icon, Button, CategoryBadge, Badge, SourceTag, AvatarStack, PosterPlaceholder } from '@/components/ui/core';

export function EventModal({ ev, store, onClose }: any) {
  ...
  return (
    <div onClick={close} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} className={`cat cat-${ev.cat}`} style={{
        ...
        {/* details */}
        <div style={{ display: "flex", flexDirection: "column", minWidth: 0, maxHeight: "92vh" }}>
          ...
          {/* action bar */}
          <div style={{ padding: "16px 24px", borderTop: "1px solid var(--border)", background: "var(--surface)", display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <div style={{ flex: 1 }}>
                <RsvpControl value={rsvp} onChange={(v: string) => store.setRsvp(ev.id, v)} />
              </div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <Button variant="soft" size="sm" icon="calendar" full onClick={() => {
                fire("Opening Google Calendar…");
                window.open(getGoogleCalendarUrl(ev), '_blank');
              }}>Add to Google Calendar</Button>
              <Button variant="soft" size="sm" icon="download" full onClick={() => {
                fire("Downloading event.ics…");
                generateICS(ev);
              }}>Download .ics</Button>
            </div>
          </div>
      ...
      {toast && (
        <div onClick={(e) => e.stopPropagation()} style={{
          position: "fixed", bottom: 26, left: "50%", transform: "translateX(-50%)", background: "var(--text)", color: "var(--surface)",
...
function InfoRow({ icon, main, sub }: any) {
  return (
...
function RsvpControl({ value, onChange }: any) {
  const opts = [
