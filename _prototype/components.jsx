/* ============================================================
   CampusPulse — shared primitives
   Logo · Icon · Button · Chip · Badge · CategoryBadge · Avatar(s)
   SegmentedControl · Toggle · Tabs · poster placeholder
   ============================================================ */
...
/* ---------- Logo: animated pulse/heartbeat mark + wordmark ---------- */
function PulseMark({ size = 28, live = true }) {
  return (
    <span style={{
...
function Logo({ size = 28, showText = true, live = true, textColor }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 10 }}>
      <PulseMark size={size} live={live} />
      {showText && (
        <span className="display" style={{ fontWeight: 700, fontSize: size * 0.66, letterSpacing: "-0.03em", color: textColor || "var(--text)" }}>
...
function Chip({ children, active, cat, onClick, dot, size = "md" }) {
...
/* ---------- Source tag (Official / Club / Community) ---------- */
...
/* ---------- Avatar + stack ---------- */
function Avatar({ person, size = 30, ring = "var(--surface)" }) {
  return (
...
function AvatarStack({ people, size = 30, max = 5, extra }) {
...
/* ---------- SegmentedControl ---------- */
function SegmentedControl({ options, value, onChange, size = "md", accent }) {
...
/* ---------- striped poster placeholder ---------- */
function PosterPlaceholder({ cat = "tech", label = "event poster", ratio = "4 / 5", radius = "var(--r-lg)", style }) {
  return (
    <div className={`cat cat-${cat}`} style={{
      ...
      <span className="mono" style={{ fontSize: 12, color: "var(--c-ink)", opacity: 0.8,
...
Object.assign(window, {
  Icon, PulseMark, Logo, Button, Chip, CategoryBadge, Badge, SourceTag,
  Avatar, AvatarStack, SegmentedControl, Toggle, Tabs, PosterPlaceholder,
});
